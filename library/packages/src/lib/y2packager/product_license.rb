...
module Y2Packager
  ...
  class ProductLicense
    ...
    class << self
      ...
      # @param handler      [:libzypp, nil] Handler for interact with license acceptance. For the
      #   time being, only :libzypp is really supported.
      #
      # @return [ProductLicense]
      def find(product_name, handler: nil, content: nil)
        ...
        cache[product_name] = ProductLicense.new(product_name, license, handler: handler)
      end
    ...
    # @param handler       [Symbol] Backend to use when syncing the licenses acceptance status
    #   (only :libzypp is supported)
    def initialize(product_name, license, handler: nil)
      @product_name = product_name
      @license = license
      @handler = Y2Packager::LicensesHandlers.for(handler, product_name) if handler
    end
